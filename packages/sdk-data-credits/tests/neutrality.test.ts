/**
 * Vertical-neutrality gate for packages/sdk-data-credits.
 *
 * A credit ledger brokers capability spend for whoever asks. It must not know which
 * industry is spending: the moment a vertical or a funnel stage appears here, the
 * budget rules belong to one tenant shape and every other vertical needs a fork.
 * Capability names, prices and approval thresholds all arrive as configuration.
 *
 * The ban list is PRECISE rather than exhaustive - every term yields zero matches
 * today, and terms that would produce false positives are excluded with the reason
 * recorded. Bare `customer` is excluded: a credit broker legitimately talks about a
 * paying customer, and a gate that fires on that gets switched off.
 */
import fs from 'fs';
...
describe('sdk-data-credits is vertical-neutral by contract', () => {
  ...
  it('CAN fail - the scanner catches a planted term in every category', () => {
    ...
    expect(scanText('export async function reserveCredits(input: ReserveInput): Promise<Reservation> {', 'synthetic.ts')).toEqual([]);
  });
});
