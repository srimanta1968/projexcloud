...
/**
 * Word-boundary patterns. Each entry is a bare term matched case-insensitively as
 * a whole word, so "leader" does not trip on "lead" and "campaign_id" does trip on
 * "campaign".
 */
const VERTICAL_TERMS = [
  ...
  'shipment', 'consignment', 'work_order', 'workorder', 'dispatch',
...
function scan(terms: string[]): Hit[] {
  const hits: Hit[] = [];
  const patterns = terms.map((t) => ({ term: t, re: new RegExp(`\\b${t}\\b`, 'i') }));
...
describe('sdk-source-record vertical-neutrality contract', () => {
  ...
  it('declares no dependency on a vertical package', () => {
    ...
    const offenders = deps.filter((d) =>
      VERTICAL_TERMS.some((t) => new RegExp(`\\b${t}\\b`, 'i').test(d)),
    );
