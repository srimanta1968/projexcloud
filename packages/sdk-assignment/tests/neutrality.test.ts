/**
 * Vertical-neutrality gate for packages/sdk-assignment.
 *
 * Routing is only reusable if it does not know what it is routing. The six-step
 * pipeline decides over a `subject_ref` and a candidate pool; the moment a stage
 * name or a job title appears in this source, the rule belongs to one vertical and
 * the next vertical forks the package instead of configuring it. Every
 * vertical-specific value arrives as a rule-set row, which is what routingService
 * already models and what publishRuleSet versions.
 *
 * The ban list is PRECISE rather than exhaustive: every term here currently yields
 * zero matches. Terms that would cry wolf are excluded with the reason recorded,
 * because a gate that fires on ordinary English gets suppressed, and a suppressed
 * gate protects nothing. Bare `lead`, `client` and `customer` are excluded for that
 * reason; the real leak patterns (`lead_source`, `lead_score`, ...) are matched.
 */
import fs from 'fs';
...
const VERTICAL_NAMES = [
  ...
  'insurance', 'healthcare', 'patient', 'clinic', 'dental', 'salon',
  'attorney', 'recruiting',
];
...
describe('sdk-assignment is vertical-neutral by contract', () => {
  ...
  it('has source to scan at all - an empty scan must not read as a pass', () => {
    ...
    expect(files.some((f) => f.endsWith('routingService.ts'))).toBe(true);
  });
  ...
  it('CAN fail - the scanner catches a planted term in every category', () => {
    ...
    expect(scanText('export async function route(input: RouteInput): Promise<RouteResult> {', 'synthetic.ts')).toEqual([]);
  });
});
