/**
 * Vertical-neutrality gate for packages/sdk-notification.
 *
 * Quiet hours, frequency caps and the dedup window are all tenant policy rows, and
 * that is the whole design: the package decides WHETHER to send, never what the
 * message means. A stage name or a job title in this source would turn a generic
 * cap into one vertical's cadence.
 *
 * Bare `customer` is deliberately NOT banned. It appears only as "customer-facing
 * email provider configuration" - a tenant bringing its own SMTP/SendGrid/SES
 * credentials - which is billing vocabulary, not funnel vocabulary. A gate that
 * fires on that would be suppressed within a week.
 */
import fs from 'fs';
...
describe('sdk-notification is vertical-neutral by contract', () => {
  ...
  it('has source to scan at all - an empty scan must not read as a pass', () => {
    ...
    expect(files.some((f) => f.endsWith('frequencyCap.ts'))).toBe(true);
  });
  ...
  it('CAN fail - the scanner catches a planted term in every category', () => {
    ...
    expect(scanText('export async function reserveSend(input: ReserveSendInput): Promise<SendDecision> {', 'synthetic.ts')).toEqual([]);
  });
});
