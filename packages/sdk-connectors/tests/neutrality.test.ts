/**
 * Vertical-neutrality gate for packages/sdk-connectors.
 *
 * The lead-form adapters are where a vertical would most easily creep in, because
 * each one parses a third party's payload. The line this gate holds is that an
 * ADAPTER may speak the provider's language while the PACKAGE must not adopt a
 * vertical's.
 *
 * `lead_id` is therefore deliberately NOT banned here, unlike in the sibling
 * packages, and the reason is specific: Meta, LinkedIn and TikTok all name that
 * field `lead_id` in their own webhook schemas, so `str(p.lead_id)` is fidelity to
 * an external contract rather than a business rule this package invented. Renaming
 * it would break the parse and prove nothing. What IS banned is the vocabulary that
 * would mean this package held opinions about the record downstream - stages, roles
 * and scoring.
 */
import fs from 'fs';
...
describe('sdk-connectors is vertical-neutral by contract', () => {
  ...
  it('has source to scan at all - an empty scan must not read as a pass', () => {
    ...
    expect(files.some((f) => f.endsWith('leadFormAdapters.ts'))).toBe(true);
  });
  ...
  it('CAN fail - the scanner catches a planted term in every category', () => {
    ...
    expect(scanText('const sourceEventId = str(p.lead_id) ?? str(p.event_id);', 'synthetic.ts')).toEqual([]);
  });
});
