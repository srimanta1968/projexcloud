/**
 * Vertical-neutrality gate for packages/sdk-conversation.
 *
 * A thread carries a `subject_ref` and a purpose string; it must not know what is
 * being discussed. The compose guardrail is the sharp edge - it returns allow /
 * review / deny but holds NO consent or policy logic of its own, delegating every
 * decision input to a caller-supplied resolver. That separation is what keeps the
 * package horizontal, and this gate guards it.
 *
 * `healthcare` is deliberately NOT banned, and the reason is worth recording: both
 * occurrences are comments asserting the package's own neutrality ("this SDK is
 * horizontal - it ships to healthcare, ..."). Banning the word would delete the
 * sentence that states the contract this file exists to enforce.
 */
import fs from 'fs';
...
const VERTICAL_NAMES = [
  ...
  'insurance', 'patient', 'clinic', 'dental', 'salon', 'attorney', 'recruiting',
];
...
describe('sdk-conversation is vertical-neutral by contract', () => {
  ...
  it('has source to scan at all - an empty scan must not read as a pass', () => {
    ...
    expect(files.some((f) => f.endsWith('threadService.ts'))).toBe(true);
  });
  ...
  it('CAN fail - the scanner catches a planted term in every category', () => {
    ...
    expect(scanText('export async function recordMessage(input: RecordMessageInput): Promise<ConversationMessage> {', 'synthetic.ts')).toEqual([]);
  });
});
